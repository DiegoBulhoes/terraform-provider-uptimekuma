...
func CreateMaintenance(ctx context.Context, c Caller, maintenance wire.Maintenance) (int, error) {
...
func UpdateMaintenance(ctx context.Context, c Caller, maintenance wire.Maintenance) error {
	if maintenance.ID == 0 {
...
// acknowledgement.
func GetMaintenance(ctx context.Context, c Caller, id int) (*wire.Maintenance, error) {
	var resp struct {
		wire.AckEnvelope
		Maintenance *wire.Maintenance `json:"maintenance"`
	}
...
func ListMaintenances(ctx context.Context, c Caller) (map[int]wire.Maintenance, error) {
...
func PauseMaintenance(ctx context.Context, c Caller, id int) error {
	return c.Mutate(ctx, c.Cache().Maintenances, nil, "pauseMaintenance", id)
}

// ResumeMaintenance reactivates a maintenance window.
func ResumeMaintenance(ctx context.Context, c Caller, id int) error {
...
func SetMaintenanceMonitors(ctx context.Context, c Caller, maintenanceID int, monitorIDs []int) error {
...
func GetMaintenanceMonitors(ctx context.Context, c Caller, maintenanceID int) ([]int, error) {
	var resp struct {
		wire.AckEnvelope
		Monitors []struct {
			ID int `json:"id"`
...
func NormalizeMaintenance(maintenance *wire.Maintenance) {
	...
	if maintenance.Active == nil {
		maintenance.Active = wire.BoolPtr(true)
	}
	...
	if len(maintenance.TimeRange) == 0 {
		maintenance.TimeRange = []wire.TimePart{
			{Hours: 0, Minutes: 0},
			{Hours: 0, Minutes: 0},
		}
	}
